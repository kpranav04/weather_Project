...
function SPI() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const res = await axios.post("http://localhost:3000/index/spi_post", {
      location: selectedOption.value,
      date: datee,
      index: data.spi_index,
      value: data.valuee,
    });
    console.log(res);
  };
  ...
  const handlegetSubmitDaily = async (e) => {
    ...
    const response = await axios.post(
      "http://localhost:3000/index/spi_get_daily",
      { dateget }
    );
    console.log(response.data);
    setMapData(response.data);
    ...
    <div className="spi">
      ...
      <div className="containera">
        ...
        <div className="filters_spi">
          ...
          <DatePicker
            selected={dateget}
            onChange={(dateget) => setgetDate(dateget)}
          />

          <button onClick={handlegetSubmitDaily}>Get Data</button>
        </div>
      </div>
    </div>
  );
